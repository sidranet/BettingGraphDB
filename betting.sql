...
-- 5. MATCH ЗАПРОСЫ (Поиск по шаблонам в графе)

-- Запрос 1: Информация о ставках конкретного игрока
-- Демонстрирует базовое соединение через MATCH (Игрок -> Ставка -> Матч -> Коэффициент)
...
-- Запрос 2: Поиск активных игроков с высокими ставками
-- Используем >= 5000, так как у топ-игроков баланс начинается ровно с этой суммы
SELECT DISTINCT p.FullName, m.Sport, c.Value
FROM dbo.Player p, dbo.PlacesBet b, dbo.Match m, dbo.HasCoefficient hc, dbo.Coefficient c
WHERE MATCH(p-(b)->m-(hc)->c) 
  AND p.Balance >= 5000 
  AND c.Value > 1.8;

-- Запрос 3: Поиск матчей, на которые ставят "лидеры мнений"
-- Демонстрирует социальную связь: за кем следит игрок 5 и на что этот "кумир" ставит
SELECT DISTINCT m.HomeTeam, m.AwayTeam, b.Status
FROM dbo.Player ps, dbo.FollowsPlayer f, dbo.Player pb, dbo.PlacesBet b, dbo.Match m
WHERE MATCH(ps-(f)->pb-(b)->m) 
  AND ps.PlayerID = 5;

-- Запрос 4: Вычисление совокупного доверия в цепочке из 3-х игроков
-- Ищем цепочку p1 -> p2 -> p3 и перемножаем их TrustWeight
SELECT p1.FullName AS StartPlayer, p2.FullName AS MiddlePlayer, p3.FullName AS EndPlayer, 
       f1.TrustWeight * f2.TrustWeight AS ChainTrust
FROM dbo.Player p1, dbo.FollowsPlayer f1, dbo.Player p2, dbo.FollowsPlayer f2, dbo.Player p3
WHERE MATCH(p1-(f1)->p2-(f2)->p3) 
  AND p1.PlayerID = 1;

-- Запрос 5: Расчет потенциального выигрыша по конкретному виду спорта
-- Демонстрирует аналитику на стыке узлов и ребер (сумма ставки * коэффициент)
SELECT p.FullName, m.HomeTeam, c.Value, (b.Amount * c.Value) AS PotentialWin
FROM dbo.Player p, dbo.PlacesBet b, dbo.Match m, dbo.HasCoefficient hc, dbo.Coefficient c
WHERE MATCH(p-(b)->m-(hc)->c) 
  AND m.Sport = 'Tennis';


-- 6. SHORTEST_PATH ЗАПРОСЫ (Поиск кратчайших путей в социальной сети игроков)

-- Запрос 1: Поиск кратчайшего пути любой длины от Иванова (ID 1) до Морозова (ID 10)
-- Используем подзапрос, чтобы избежать ограничений парсера на работу с коллекциями FOR PATH
SELECT 
    StartPlayer, 
    EndPlayer, 
    Path
FROM (
    SELECT 
        ...
        LAST_VALUE(p2.PlayerID) WITHIN GROUP (GRAPH PATH) AS EndPlayerID,
...
WHERE EndPlayerID = 10;

-- Запрос 2: Поиск пути с ограничением по количеству шагов
-- В нашей базе путь от 1 до 10 длинный, поэтому ставим лимит до 10 шагов {1,10}
-- COUNT(p2.PlayerID) позволяет вычислить реальное количество рукопожатий между игроками
SELECT 
    StartPlayer, 
    EndPlayer, 
    Steps, 
